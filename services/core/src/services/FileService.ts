...
import { FileObject, Match, Tag, MatchLevel } from '../utils/types';
...
export interface IFileService {
    ...
    getTree(chainId: any, address: string, match: string): Promise<Array<string>>;
    getContents(chainId: any, address: string, match: string): Promise<Array<FileObject>>;
}
...
export class FileService implements IFileService {
    ...
    async getTree(chainId: any, address: string, match: MatchLevel): Promise<Array<string>> {
        chainId = getChainId(chainId);
        const fullMatchesTree = this.fetchAllFileUrls(chainId, address, "full_match");
        if (fullMatchesTree.length || match === "full_match") {
            return fullMatchesTree;
        }
        return this.fetchAllFileUrls(chainId, address, "partial_match");
    }

    async getContents(chainId: any, address: string, match: MatchLevel): Promise<Array<FileObject>> {
        chainId = getChainId(chainId);
        const fullMatchesFiles = this.fetchAllFileContents(chainId, address, "full_match");
        if (fullMatchesFiles.length || match === "full_match") {
            return fullMatchesFiles;
        }
        return this.fetchAllFileContents(chainId, address, "partial_match");
    }
